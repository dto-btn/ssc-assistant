...
<<<<<<< HEAD
  ...
  it("moves annotation citations outside bare urls that markdown later autolinks", () => {
    const text = "See https://example.com/portal for details.";
    const urlMiddle = text.indexOf("example") + "example".length;
    const citations: Citation[] = [
      {
        title: "Portal Source",
        url: "https://example.com/portal",
    ...
    expect(result.processedText).toContain(
      "https://example.com/portal [1](<https://example.com/portal>) for details.",
    );
    expect(result.processedText).not.toContain("https://example [1]");
    expect(result.processedText).not.toContain("https://example.com/[1]");
  });

  it("preserves explicit numeric citation markdown links without inserting duplicate markers", () => {
    const text = "Please review [1](https://example.com/shared.pdf) and [2](https://example.com/shared.pdf).";
    const citations: Citation[] = [
      {
