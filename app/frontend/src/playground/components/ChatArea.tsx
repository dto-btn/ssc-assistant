...
import { closeChatFeedbackModal } from "../store/slices/uiSlice";
...
const ChatArea: React.FC<ChatAreaProps> = ({ onOpenSidebar, isSidebarOpen }) => {
  ...
  const isLoading = useAppSelector((state) =>
    currentSessionId
      ? (state.chat.isLoadingBySessionId[currentSessionId] ?? false)
      : false,
  );
  ...
  const isNewChat = useAppSelector(
    (state) =>
      state.sessions.sessions.find((s) => s.id === currentSessionId)
        ?.isNewChat ?? false,
  );
  ...
  const chatFeedbackModalState = useAppSelector(
    (state) => state.ui.chatFeedbackModalState,
  );
  ...
  const handleExport = React.useCallback(
    async (format: PlaygroundExportFormat) => {
      if (!currentSessionId) {
        dispatch(
          addToast({
      ...
      try {
        ...
        if (format === "json") {
          ...
          await downloadSessionExportPdf(
            activeSession.name,
            exportDocument,
      ...
      >
        ...
        >
          <CircularProgress
            size={40}
            sx={{ mb: 2 }}
            aria-describedby={hydrationStatusMessageId}
          />
          <Typography
            id={hydrationStatusMessageId}
            variant="body1"
    ...
    >
      ...
      >
        ...
        {chatFeedbackModalState && (
          <ChatFeedbackForm
            open={Boolean(chatFeedbackModalState)}
            onClose={() => dispatch(closeChatFeedbackModal())}
            messageId={chatFeedbackModalState.messageId}
            sessionId={chatFeedbackModalState.sessionId}
          />
        )}
        <OrchestratorDebugPanel sessionId={currentSessionId} />
        <ChatInput sessionId={currentSessionId} />
