...
def test_files_for_session_paginates_latest_sessions(monkeypatch, api_headers, test_client):
    container = FakeContainerClient("https://example.com/assistant-chat-files-v2")
    sessions = [
        ("session-1", "2023-01-01T00:00:00Z"),
        ("session-2", "2023-01-02T00:00:00Z"),
        ("session-3", "2023-01-03T00:00:00Z"),
    ]

    for session_id, timestamp in sessions:
        container.add_blob(
            FakeBlob(
                f"user-123/chat/{session_id}.chat.json",
                {
        ...
        container.add_blob(
            FakeBlob(
                f"user-123/files/{session_id}/attachment.txt",
                {
                    "sessionid": session_id,
                    "uploadedat": timestamp,
                    "originalname": "attachment.txt",
    ...
    assert {entry["sessionId"] for entry in first_payload["files"]} == {"session-3", "session-2"}
    ...
    assert {entry["sessionId"] for entry in second_payload["files"]} == {"session-1"}
